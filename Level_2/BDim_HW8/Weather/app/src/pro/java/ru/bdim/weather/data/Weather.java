...
// и классами-преставителями источников данных (InternetWeather и DatabaseWeather)
    ...
    public static void refresh(Context context){
        instance.dataListener.loading();
        Log.d(TAG, "Загрузка по текущему местоположению");
        final String lang = context.getResources().getString(R.string.lang);
        GeoLocation.getCoord(context, new GeoLocation.GeoListener() {
            @Override
            public void coordSuccess(double lat, double lon) {
                Log.d(TAG, String.format(Locale.ROOT, "lat %.2f, lon %.2f", lat, lon ));
                instance.binder.getCurrentWeatherByCoord(lat, lon, lang, instance.connectionListener);
                instance.binder.getHourlyForecastByCoord(lat, lon, instance.connectionListener);
            }
            @Override
    ...
    public static void refresh(Context context, double lat, double lon) {
        instance.dataListener.loading();
        Log.d(TAG, "загрузка по координатам lat = " + lat + ", lon = " + lon);
