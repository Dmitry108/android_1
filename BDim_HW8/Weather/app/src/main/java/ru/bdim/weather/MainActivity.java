...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //температура
        double t = WeatherData.getData().getMain().getTemperature();//rn.nextInt(61)-30;
...
//        ivSky.setImageResource(imgSkyArray[cl]);
        ...
        if(settings) {
            ...
            int windSpeed = WeatherData.getData().getWind().getSpeed();//rn.nextInt(15);
            int windDirection = windSpeed == 0 ? 0:
                    WeatherData.getData().getWind().getDeg()/8 + 1;//rn.nextInt(8) + 1;
            int humidity = WeatherData.getData().getMain().getHumidity();//rn.nextInt(101);
            int pressure = WeatherData.getData().getMain().getHumidity();//rn.nextInt(101) + 700;
